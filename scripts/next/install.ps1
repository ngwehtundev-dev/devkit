function Install-NextDevKit {
  ...
  Write-Host "Professional Next.js starter installer" -ForegroundColor DarkGray
  Write-Host ""

  if ($Preset -ne "standard") {
    Fail "Only -Preset standard is supported in this version."
  }
  ...
  if (-not (Test-Command $PackageManager)) {
    if ($PackageManager -eq "pnpm") {
      ...
      try {
        ...
        Write-Warn "Corepack setup failed."
      }
    }
  }
  ...
  Write-Step "Creating Next.js project: $ProjectName"
  ...
  Write-Step "Creating professional folder structure..."
  ...
  Write-Step "Applying standard preset files..."
  ...
  Copy-TemplateFile "src/components/shared/site-footer.tsx"
  ...
  if (-not $NoInstall) {
    Write-Step "Installing standard dependencies..."
  ...
  Write-Host "  ✓ ESLint"
  Write-Host "  ✓ Prettier"
  Write-Host "  ✓ Professional src structure"
